...
insert into Professionals(sr,First_Name,Last_Name,Salary,Joining_Date,Department)values
...
select first_name ,salary ,department from professionals p ;
...
select `First_Name`,`Last_Name`,`Salary` from professionals where `First_Name`="Virat" 
and `Last_Name`="Jadeja";

select `First_Name`,`Last_Name`,`Salary` from professionals where `Salary`=500000 
and `Department`="Architect";
select `First_Name`,`Last_Name`,`Salary` from professionals where 'salary'<300000;
select * from  professionals where 'salary'<300000 
and `Department`="ENGINEER";

select * from  professionals where Department = 'sales' 
or Department = 'Finance';

select * from  professionals where Department not in(sales and finance)  ;
...
select substring(first_name,1,1) from professionals;
...
select First_Name,INSTR(First_Name,'a') from professionals; where First_Name ="Amitabh";
select INSTR(First_Name,'t') from professionals where
First_Name ="Amitabh";

select INSTR(binary First_Name,'a') from professionals where First_Name ="Amitabh";

#6 Write An SQL Query To Print The FIRST_NAME From Professionals Table After Removing White Spaces From The Right Side
select TRIM(Department) from professionals;
...
select distinct  RTRIM(Department), length( RTRIM(Department)) from 
professionals;
...
select * from professionals order by ltrim(First_Name) asc;

select * from professionals order by Salary desc , first_name desc;
...
select * from professionals order by Salary desc ,department desc ;

select * from professionals order by first_name  desc;
select * from professionals order by ltrim(First_Name) asc,Department desc;
...
#15 Write An SQL Query To Print Details With DEPARTMENT Name As “Admin"(WILD CARD)
select * from professionals where Department like "E%";
select * from professionals where first_name like "Sh%";

#16 Write An SQL Query To Print Details Whose FIRST_NAME Ends With ‘A’ (WILD CARD)
select * from professionals where First_Name like "%t";

#First Name start with a
select * from professionals where First_Name like "a%";

#17 Write An SQL Query To Print Details Whose FIRST_NAME Contains ‘a’ (WILD CARD)
select * from professionals where First_Name like "%am%";

#18 Write an SQL Query to fetch name of people whose name starts with S and ends with L(WILD CARD)
select * from professionals where last_name like  "s%a";

#19 (OFFSET)Write An SQL Query To Print Details Whose FIRST_NAME Ends With ‘m’ And Contains Six Alphabets 
...
select * from professionals where salary like '2_____';
...
select * from professionals where joining_date between "2021-02-20 09:00:00" and "2021-11-11 09:00:00";
...
Select * from professionals where month(JOINING_DATE) >= 4 and month(JOINING_DATE) <=10
and day(JOINING_DATE) = 11 and day (Joining_date) >= 11 and day(Joining_date) <= 21
...
select * from 
(select * from professionals where Department like "E%") Engineer_Table ;

#22 SubQuery
# Write An SQL Query To Print Details Whose SALARY Lies Between 100000 And 500000 (Same as Q20)
SELECT professional_ID, FIRST_NAME, LAST_NAME, Salary 
FROM professionals  
...
# Max Salary
select max(salary) from Professionals ;
#select salary from professionals where salary not in (500000) :
select max(salary) from professionals where salary
not in (select max(salary) from professionals);

select max(salary) from Professionals
where
salary not in ((select max(salary) from Professionals),(select max(salary) from professionals where salary
not in (select max(salary) from professionals)));

#4th max
select max(salary) from Professionals
where salary not in ((select max(salary) from Professionals),(select max(salary) from professionals where salary
not in (select max(salary) from professionals)),(select max(salary) from Professionals
where
salary not in ((select max(salary) from Professionals),(select max(salary) from professionals where salary
not in (select max(salary) from professionals)))));
...
#5th max
select max(salary) from professionals where salary
not in((select max(salary) from professionals),(select max(salary) from professionals where salary
not in (select max(salary) from professionals)),(select max(salary) from Professionals
where
salary not in ((select max(salary) from Professionals),(select max(salary) from professionals where salary
not in (select max(salary) from professionals)))),(select max(salary) from Professionals
...
#2nd min
select  min(salary) from Professionals 
where salary not in (select min(salary) from Professionals);
...
select department, sum(salary) from professionals
...
select department, sum(salary) as sum_sal from professionals
group by department order by sum_sal desc ;

#23 Group By
#Write An SQL Query To Fetch The No. Of people For Each Department In The Descending Order
SELECT first_name, count(*) No_Of_Workers, avg (salary), max(salary)  
FROM professionals  
GROUP BY first_name 
ORDER BY No_Of_Workers DESC; 

SELECT last_name, count(*) No_Of_Workers, avg (salary), max(salary)  
FROM professionals  
GROUP BY last_name
ORDER BY No_Of_Workers desc ;
...
#floor function
SELECT * FROM professionals WHERE floor (professional_ID/ 2) > 2;
...
select * from professionals order by Salary desc limit 3;

select * from professionals order by Salary desc limit 6,3;
...
#above all are DQL commands

#Alter(DDL,data defination command)
...
#Rename
alter table professionals rename column professinal_id to sr;
...
select *
from customers m inner join orders t
...
select * from products p cross join orders o; # use when we want best combination of tables

#Point to be noted here is, first id column is for products table, while other is for orders table.

#Multiple(more than 2 columns) join-->Combine all the 3 tables
...
SELECT name FROM customers WHERE name like 'J%';#(wildcard)
...
SELECT email FROM customers WHERE email like '%@gmail.com';#(wildcard)
...
SELECT name FROM customers WHERE name like '%Jo%'or name like'%ee%'
or name like  '%lli%';#(wildcard)
...
select first_name, last_name, department from professionals;
...
create procedure professional_detail (in x int)
select * from professionals where sr=x;
select * from professionals;

#Q.65 Call and drop stored procedure
call professional_detail (1);
drop procedure professional_detail;
